...
pub fn parse_select_with_semi_colon() {
...
pub fn parse_delete_with_semi_colon() {
    let sql: &str = "DELETE FROM 'table';";
...
pub fn verified(query: &str) -> ASTNode {
    let ast = parse_sql(query);
    assert_eq!(query, &ast.to_string());
    ast
}

pub fn parses_to(from: &str, to: &str) {
    assert_eq!(to, &parse_sql(from).to_string())
}

pub fn joins_from(ast: ASTNode) -> Vec<Join> {
    match ast {
...
pub fn parse_sql(sql: &str) -> ASTNode {
...
pub fn parse_sql_with(sql: &str, dialect: &Dialect) -> ASTNode {
